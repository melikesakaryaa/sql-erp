...
GO
-- --------------------------------------------------------------------------------------------
create PROCEDURE [dbo].[pdxCreateFKIndexes]
AS
 ...
 Declare pk_cursor CURSOR FAST_FORWARD for 
   ...
   WHERE COLUMNS.COLUMN_NAME LIKE 'FK[_]%' AND COLUMNS.TABLE_NAME=TABLES.TABLE_NAME AND
         ...
         NOT EXISTS(SELECT 1 FROM SYS.INDEXES 
	             WHERE NAME = 'IxFK_'+COLUMNS.TABLE_NAME+'_'+COLUMNS.COLUMN_NAME);
 ...
 BEGIN TRANSACTION
   ...
   BEGIN
      select @Index_name = 'IxFK_' + @table_name + '_' + @column_name;
      select @SQL = 'CREATE NONCLUSTERED INDEX ' + @Index_name + ' ON ' + @table_name + ' (' + @column_name + ' ASC )' ;
      EXEC(@SQL);
      -- next record
      FETCH NEXT FROM pk_cursor INTO @table_name, @column_name
   END
